  ...
  run: async (client, message, args, prefix) => {
    ...
    let r1 = new MessageActionRow().addComponents(
      new MessageSelectMenu()
        .setCustomId(`he`)
        .setPlaceholder(`> ${client.user.username} Command Categories`)
        .addOptions([
          {
            label: `Home`,
    ...
    let r2 = new MessageActionRow().addComponents(
      new MessageButton()
        .setStyle(`PRIMARY`)
        .setCustomId(`lol1`)
        .setEmoji(`${client.emoji.left2}`),
      new MessageButton()
        .setStyle(`SECONDARY`)
        .setCustomId(`lol2`)
        .setEmoji(`${client.emoji.left1}`),
      new MessageButton()
        .setStyle(`DANGER`)
        .setCustomId(`lol3`)
      ...
      new MessageButton()
        .setStyle(`SECONDARY`)
        .setCustomId(`lol4`)
        .setEmoji(`${client.emoji.right1}`),
      new MessageButton()
        .setStyle(`PRIMARY`)
        .setCustomId(`lol5`)
        .setEmoji(`${client.emoji.right2}`),
    );
        ...
        for (const value of interaction.values) {
          if (value === `h1`) {
            let em = new MessageEmbed()
              ...
              .setFooter({
                text: `Just A Powerful Discord bot`,
    ...
    collector.on("end", async () => {
      msg.edit({
        embeds: [em],
        components: [],
        content: `${client.emoji.info} | Help Query Got Timed Out!`,
      });
    });
  },
};
